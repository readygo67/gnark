...
func TestInnerRSACircuit(t *testing.T) {
	innerCcs, _, _, _ := computeInnerProof(ecc.BN254.ScalarField(), ecc.BN254.ScalarField())
	fmt.Printf("innerCs: %+v\n", innerCcs)
}

func TestRecursionRSA_1(t *testing.T) {
	// compute the proof which we want to verify recursively
	innerCcs, innerVK, innerWitness, innerProof := computeInnerProof(ecc.BW6_761.ScalarField(), ecc.BN254.ScalarField())
...
func TestRecursionRSA_2(t *testing.T) {
	// compute the proof which we want to verify recursively
