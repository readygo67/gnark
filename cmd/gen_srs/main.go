...
func main() {
	...
	//}
	{
		bn254Field := ecc.BN254.ScalarField()
		bn254Tau, err := rand.Int(rand.Reader, bn254Field)
		if err != nil {
			panic(err)
		}
		fmt.Printf("bn254 tau: %v\n", bn254Tau)
		for i := 16; i < 28; i++ {
			generateSRSStart := time.Now()
			srsFile := fmt.Sprintf("bn254_pow_%v.srs", i)
			lSrsFile := fmt.Sprintf("bn254_pow_%v.lsrs", i)

			size := 1<<uint(i) - 1
			srs, lsrs, err := unsafekzg.NewSRSWithSize(size, bn254Field, unsafekzg.WithTau(bn254Tau))
			if err != nil {
				panic(err)
			}
			...
			generateSRSDuration := time.Since(generateSRSStart)
			fmt.Printf("%v generate srs duration: %v\n", srsFile, generateSRSDuration)
		}
	}
